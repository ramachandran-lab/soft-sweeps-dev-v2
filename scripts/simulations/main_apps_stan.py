...
# Main script for standardizing application data     # 
#----------------------------------------------------#
#!/usr/bin/env python
...
if __name__ == '__main__':
    ...
    features_file = '../../data/applications/1000GP_Phase1_stan/{0}_chr{1}_pop{2}_features.txt'.format(label, chrm, pop)
    metadata_file = '../../data/applications/1000GP_Phase1_stan/{0}_chr{1}_pop{2}_metadata.txt'.format(label, chrm, pop)
    ...
    features = get_standardize(params_shared, data_shared, meta_shared, hapbin=True)
    out_metadata_app(metadata, metadata_file)
    out_features_app_standardize(features, features_file)
